...
def rosen_univariate(X, dim, N_DIM):
    ...
    """
    # # Old version: Low Fidelity distorted
    ...
    # cut through origin (middle of design space when symmetric bounds)
    Xuni = np.zeros(X.shape)
    Xuni[:, dim] = X[:,dim]
    Y = rosenbrock(Xuni)
...
def get_rosen_emulator(dim, N_DIM):
    ...
    """
    emulator = lambda X : rosen_univariate(X, dim, N_DIM)
    return(emulator)

def rosen_bivariate(X, dims, N_DIM):
    """
    A Low Fidelity bivariate version of the Rosenbrock function
    X: A 2D numpy array (MUST BE 2D)
    dims (list or tuple): The dimensions selected for the bivariate 
                          function (0 to N_DIM-1)
    N_DIM: The total number of dimensions
    """

    # cut through origin (middle of design space when symmetric bounds)
    Xbiv = np.zeros(X.shape)
    d1, d2 = dims
    Xbiv[:, d1] = X[:,d1]
    Xbiv[:, d2] = X[:,d2]
    Y = rosenbrock(Xbiv)
...
def get_rosen_biv_emulator(dims, N_DIM):
    """
    Returns a lambda function for use as an emulator
    """
    emulator = lambda X : rosen_bivariate(X, dims, N_DIM)
    return(emulator)
...
def fun3d_simulation_normx(x, out_name, subfolder):
    ...
    #lb = np.array([1.2, -5., 0.]) #-2.
    ub = np.array([6.0,  15., 50_000.])  # unitless, deg, meters
    lb = np.array([1.2, -5., 0.])  #
    ...
    def edit_input(x):
        ...
        atmos = Atmosphere(x_height)
        T = atmos.temperature[0]  #kelvin
        p = atmos.pressure[0]  #pascal
        ...
        # Re is defined according to the grid 
        # (which is 1 meter, although geometry is converted from in.)
        Len = 1.0   #4.47 # Aircraft length (meters)
        Re = Len*vel*rho/mu
        
        # dynamic pressure (useful for getting forces and moments)
        # from https://en.wikipedia.org/wiki/Dynamic_pressure
        # (compressible isentropic flow)
        # a more involved alternative equation is: 
        # https://www.grc.nasa.gov/www/BGH/isentrop.html
        q = p*(1+0.5*(gamma-1)*x_mach**2)**(gamma/(gamma-1))-p
...
def fun3d_simulation_cl(xdat, out_name, subfolder):
    """ coefficient of lift """
    ndat = xdat.shape[0]
    CL = np.zeros([ndat, 1])
    CD = np.zeros([ndat, 1])
    for ii in range(ndat):
        xii = xdat[ii,:]
        CL[ii], CD[ii], _ = fun3d_simulation_normx(xii, out_name, subfolder)
    return(CL)


def fun3d_simulation_cd(xdat, out_name, subfolder):
    """  coefficient of drag  """
    ndat = xdat.shape[0]
    CL = np.zeros([ndat, 1])
    CD = np.zeros([ndat, 1])
...
def fun3d_simulation_cl_cd(xdat, out_name, subfolder):
    """ lift to drag ratio """
    ndat = xdat.shape[0]
    CL = np.zeros([ndat, 1])
    CD = np.zeros([ndat, 1])
...
def fun3d_simulation_lift_force(xdat, out_name, subfolder):
    """ lift (N) """
    ndat = xdat.shape[0]
    CL = np.zeros([ndat, 1])
    CD = np.zeros([ndat, 1])
    ...
    for ii in range(ndat):
        xii = xdat[ii,:]
        CL[ii], CD[ii], q[ii] = fun3d_simulation_normx(xii, out_name, subfolder)
    Aref = 1  #m**2, reference area
    Lift = CL*q*Aref
    return(Lift)
